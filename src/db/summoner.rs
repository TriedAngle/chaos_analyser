...
pub fn get_by_name(summoner_name: &str, conn: &PgConnection) -> QueryResult<Vec<Summoner>> {
    all_summoners
...
pub fn get_by_name_and_region(summoner_name: &str, summoner_region: &str, conn: &PgConnection) -> QueryResult<Summoner> {
    all_summoners
        .filter(summoners::name.eq(summoner_name).and(summoners::region.eq(summoner_region)))
        .get_result::<Summoner>(conn)
}
