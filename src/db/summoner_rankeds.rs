...
pub fn get_all(conn: &PgConnection) -> QueryResult<Vec<SummonerRanked>> {
...
pub fn get_by_summoner_id(summoner_id: i64, conn: &PgConnection) -> QueryResult<SummonerRanked> {
    all_summoner_rankeds
        .find(summoner_id)
...
pub fn insert_summoner_ranked(new_ranked: NewSummonerRanked, conn: &PgConnection) -> bool{
    if check_existing_summoner_ranked(new_ranked.summoner_id, conn) {
        update_summoner_ranked(new_ranked, conn)
    } else {
...
pub fn update_summoner_ranked(new_ranked: NewSummonerRanked, conn: &PgConnection) -> bool {
