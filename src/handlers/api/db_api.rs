...
pub async fn get_summoner_by_id(req: HttpRequest, pool: web::Data<DbPool>) -> HttpResponse {
    ...
    let id: i64 = req.match_info()
        .get("id")
        .unwrap_or("1")
        .parse().unwrap();

    HttpResponse::Ok().json(db_summoner::get_by_id(id, &conn).await.unwrap()).await.unwrap()
}
...
pub async fn get_summoner_by_puuid(req: HttpRequest, pool: web::Data<DbPool>) -> HttpResponse {
    ...
    let puuid: &str = req.match_info()
        .get("puuid")
        .unwrap();

    HttpResponse::Ok().json(db_summoner::get_by_puuid(puuid, &conn).await.unwrap()).await.unwrap()
}

/// Returns a single Summoner as the name is unique for each region
pub async fn get_summoner_by_name_and_region(req: HttpRequest, pool: web::Data<DbPool>) -> HttpResponse {
    let conn: r2d2::PooledConnection<ConnectionManager<PgConnection>> =
        pool.get().expect("couldn't get db connection from pool");
    ...
    HttpResponse::Ok().json(db_summoner::get_by_name_and_region(summoner_name, summoner_region, &conn).await.unwrap()).await.unwrap()
}
...
pub async fn get_summoners_by_name(req: HttpRequest, pool: web::Data<DbPool>) -> HttpResponse {
    ...
    HttpResponse::Ok().json(db_summoner::get_by_name(summoner_name, &conn).await.unwrap()).await.unwrap()
}
...
pub async fn get_summoner_ranked_by_summoner_id(req: HttpRequest, pool: web::Data<DbPool>) -> HttpResponse {
    let conn: r2d2::PooledConnection<ConnectionManager<PgConnection>> = 
        pool.get().expect("couldn't get db connection from pool");
    ...
    HttpResponse::Ok().json(db_summoner_rankeds::get_by_summoner_id(summoner_id, &conn).await.unwrap())
}
